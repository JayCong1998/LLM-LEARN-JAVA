 ...
 */
@Component // 注册为生产环境中负责工具可靠性策略的 Spring Bean。
@Primary // 让 ManualReactAgent 注入 ToolExecutionPort 时优先使用带重试的包装器。
public class RetryingToolExecutor implements ToolExecutionPort { // 定义只处理超时重试的生产工具执行端口。
    ...
    public String execute(String toolName, String arguments, ToolRetryListener retryListener) { // 执行首次调用，并仅在精确超时时安排有限重试。
        String observation = delegate.execute(toolName, arguments); // 先执行首次工具调用，保持所有正常与业务失败 Observation 原样可见。
        for (int attempt = 2; attempt <= MAX_ATTEMPTS && isTimeout(observation, toolName); attempt++) { // 仅在上一轮为精确超时时进入第二或第三次尝试。
            ...
            observation = delegate.execute(toolName, arguments); // 退避正常完成后执行下一次受单独三秒限制的工具调用。
        } // 结束最多两次的超时重试循环。
