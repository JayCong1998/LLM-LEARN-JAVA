...
import com.jaycong.know.engine.document.constant.ContentType;
...
public class MarkdownProcessServiceImpl extends MinerUProcessBaseServiceImpl {
    ...
    public String processDocument(KnowledgeDocument document, InputStream inputStream) {
        ...
        // 更新状态为转换中
        knowledgeDocumentService.updateStatus(document.getId(), DocumentStatus.CONVERTING);
        ...
        try {
            ...
            // 4. 更新文档状态为已转换
            knowledgeDocumentService.updateStatus(document.getId(), DocumentStatus.CONVERTED);
            ...
            knowledgeDocumentService.updateStatus(document.getId(), DocumentStatus.UPLOADED);
            throw new BusinessException(ErrorCode.INTERNAL_ERROR, "Markdown 文档处理失败: " + e.getMessage());
        } finally {
